...
namespace digilent {
  ...
  public:
    AXI_VDMA(uint16_t dev_id, uint32_t frame_buf_base_addr, IrptCtl& irpt_ctl, uint16_t rd_irpt_id, uint16_t wr_irpt_id) :
      rd_err_handler_(std::bind(&AXI_VDMA::readErrorHandler, this, _1)),
      wr_err_handler_(std::bind(&AXI_VDMA::writeErrorHandler, this, _1)),
      context_{},
    ...
    void enableRead(uint16_t h_res, uint16_t v_res)
    {
      XStatus status;
    ...
    void enableWrite(uint16_t h_res, uint16_t v_res)
    {
